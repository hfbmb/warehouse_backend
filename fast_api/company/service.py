...
# Set up logging for the company_service module
logging.getLogger("company_service")
...
async def update_data(name: str, data: dict):
    try:
        ...
        logging.error(f"Update_data func company allready exist %s",e)
        raise CompanyAlredyExist()
...
async def update_data_warehouse(name: str, data: dict):
    ...
    if not update_result:
        logging.error("Update warehouse data function error")
...
async def add_warehouse(company_name: str, data: dict):
    ...
    if not update_result.matched_count:
        logging.error("Add warehouse to company error: matched_count")
        raise WarehouseNotFound()
...
async def get_unsuitable_place_by_id(place_id: str) -> dict:
    ...
    if not place_data:
        logging.error("get_unsuitable_place_by_id: Not found unsuitable place by id")
        raise DoesNotExist
